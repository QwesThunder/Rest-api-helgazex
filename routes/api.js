...
var len = 15
        var arr = '12345F6789abcdefghijklmnopqrstuvwxyz'
        var random = '';
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'ZasXCar') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'ZasXCar')  return res.json(loghandler.invalidKey)
